...
#+CAPTION: Clock summary at [2019-02-05 Tue 16:54]
| Headline     | Time   |
|--------------+--------|
| *Total time* | *0:00* |
#+END:
...
(setf (symbol-function 'test1) (lambda (n) (+ 1 (* 3 n))))
(test1 2)
...
#+end_note
#+begin_info
The first evaluated as expected with a result of 6.
The second did not evaluate becuase "d" was not defined.
The third evaluated and printed "[+ a b c d]" becuase it was in brackets.
The fourth evaluated and resulted in 10, despite the fact that I though eLisp couldn't handle operator overloading.
the fifth did not evaluate becuase "a" was a void variable.
the sixth evaluated becuase the "*" allowed a to evaluate as a variable.
#+end_info
** DONE TEJ
...
#+end_note
#+begin_info
 When you remove those parts, it changes the key values to "nil".
#+end_info
** DONE TER
   CLOSED: [2019-01-24 Thu 13:34]
#+begin_note
...
#+BEGIN_SRC emacs-lisp :results silent
  ...
  (morphifyr 'downcase '("ABCDE" "9876"))
#+END_SRC

  Explain how this version of =morphify= works.
#+end_note
#+begin_info
  It looks like it manipulates the text depending on what modifier the user chooses.
  Morphifyr returns nil if nothing is passed to it, and calls itself recursively instead of a loop.
#+end_info
** DONE TGE
   CLOSED: [2019-01-24 Thu 13:40]
#+begin_note
...
#+end_note
#+begin_info
  It does not look like it, becuase it is returning what looks like a floating point number
  instead of an integer.  If it returned "3" like the regular floor function, It's correct.
#+end_info
** DONE THP
   CLOSED: [2019-01-24 Thu 13:48]
#+begin_note
...
#+end_note
#+begin_info
  I attempted to do this with ascii, but it proved too much work and I really want to finish this assignment.
  You can view my graphs at THPfloor.png and THPceil.png.
#+end_info
** DONE WKM
...
#+BEGIN_SRC emacs-lisp
(defun split-num (num)
(- num (floor num))) ;; got help with this part.  split the decimal from the rest of the number

(defun round (num)
   (cond ;; try until one works
      ((< (split-num num) .5) (floor num)) ;; if the fraction part is above .5, ceiling, below, floor
      (t (ceiling num))
   )
)   
...
** DONE TKG
   CLOSED: [2019-01-24 Thu 14:15]
#+begin_note
  Explore the inverse of =assoc= via the functions =rassoc= and associates.
#+end_note
#+begin_info
  Implemented it in WFK.
#+end_info
** DONE WOV
   CLOSED: [2019-01-24 Thu 14:24]
#+begin_note
...
#+begin_info
  1. Add 7 each time
  2. Add 4 each time
  3. Add n+1 each time (eg: add 1, then 2, 3, 4, etc.)
  4. Binary counter (decimal: 1, 2, 3, 4, 5, etc.
  5. Fibonacci sequence 
  6. n, n*3+1, n/2 repeating 
  7. Another Fibonacii 
  8. *2 each time
  9. *3 each time
  10. prime numbers
...
#+begin_info
  ...
  | =evenp=    |      yes   |
  | =oddp=     |      yes   |
Pretty much anything with a "p" is a predicate.
#+end_info
** DONE WRP
   CLOSED: [2019-01-24 Thu 14:33]
#+begin_note
...
#+end_note
#+begin_info
  1. There exists a person that is older than 21.
  2. Every person (all) is older than 21.
  3. There exists a person that is not older than 21.
  4. Ever person (all) is not older than 21
#+end_info
** DONE TOJ
   CLOSED: [2019-01-24 Thu 14:37]
#+begin_note
...
#+end_note
#+begin_info
  1. There exists a student of BYU-Idaho in the residents of Idaho.
  2. Every resident in Idaho is a student of BYU-Idaho.
  3. A student of BYU-Idaho does not exist among the residents of Idaho.
  4. There are residents of Idaho that are not students of BYU-Idaho
  5. Not every resident of Idaho is not a student of BYU-Idaho.
  6. Every resident of Idaho is not a student of BYU-Idaho.
#+end_info
** DONE WRU
   CLOSED: [2019-01-24 Thu 15:06]
#+begin_note
...
#+end_note
#+begin_info
  1. Every friend is cool.
  2. There exists a person that  is a friend and is cool.
  3. Everyone is a friend and is cool
  4. There exists a person that is a cool friend.
#+end_info
** DONE TOL
...
#+end_note
#+begin_info
  1. There exists an x that is greater than every y.
  2. There exists an x and a y that if x is greater than or equal to zero and y 
     is greater than or equal to zero, then x times y is greater than or equal to zero.
  3. There exists an x that is equal to every y plus a z the exists.
#+end_info
** DONE WVQ
   CLOSED: [2019-01-24 Thu 15:18]
#+begin_note
...
#+end_note
#+begin_info
  1. Every student in the class asks a student that exists in the class a question.
  2. Every student in the class askes a question to every student in the class.
  3. There exists a student in the class that asks another student that exists in the class a question.
  4. There exists a student in the class that every student in the class a question
  5. Every student in the class is asked a question by a student that exists in the class.
  6. A student that exists in the class is asked a question by a student that exists in the class.
#+end_info
** DONE TUQ
...
#+begin_info
  ...
  3. \forall x \ni y T(x, y)
  4. \ni x \forall y T(x, y) \land (x \ge 2)
#+end_info
** DONE TVL
   CLOSED: [2019-01-24 Thu 15:44]
#+begin_note
...
#+begin_info
  ...
  2. No person in the domain has dropped out of every class.
  3. Every student has dropped out of a class that exists.
#+end_info
** DONE TXG
   CLOSED: [2019-01-24 Thu 16:14]
#+begin_note
...
#+end_info
#+BEGIN_SRC elisp
(defun vector-to-string(vec) ;; roomate told me it existed on the mini-primer!
   (append vec nil))

(format "%s" (vector-to-string [a dog is]))
...
#+end_info
** DONE WJS
   CLOSED: [2019-01-24 Thu 16:40]
#+begin_note
  Describe an analogous situation to floor when taking the ceiling of negative
  numbers.
#+end_note
#+begin_info
  When a number is negative and you take the ceiling, it rounds up to the higher negative number.
  When you take the floor of a negative, it rounds down to the next negative number.  
  For example, ceiling (-5.2) is -5 and floor of (-5.2) is -6.
#+end_info
** DONE WKC
...
#+end_note
#+begin_info
  Online, it says the counterpart to Sum() is Product(), which comes with the symbol \pi
#+end_info
** DONE WRD
   CLOSED: [2019-01-25 Fri 15:41]
#+begin_note
...
#+BEGIN_SRC elisp
(require 'cl)
(loop for k from 0 to 5
      sum (loop for i from 0 to k
                sum (* k k i)))
...
#+begin_info
  1. x = -2.08 ish
  2. all positive numbers greater than 1
  3. true
  4. false (all positive numbers)
#+end_info
** TODO WUZ
#+begin_note
